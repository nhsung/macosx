...
START(){
  echo "Starting tmate as detached session..." 2>&1 >> /tmp/tmatemaster.log
  #$TMATE -S /tmp/tmate.sock new-session -d               # Launch tmate in a detached state
  $TMATE -S /tmp/tmate.sock new-session                  # Launch tmate in a detached state
  $TMATE -S /tmp/tmate.sock wait tmate-ready             # Blocks until the SSH connection is established 
  NOTIFY
}

STOP(){
  echo "Stoping tmate (killing processes)..." 2>&1 /tmp/tmatemaster.log 
...
PROCMON(){
  ...
  if [ "$STATUS" == 0 ]; then
    echo "tmate is stopped, starting that..." 2>&1 >> /tmp/tmatemaster.log
    START
  fi
}
...
NOTIFY(){
  ...
  if [ "$TMATECONSTR" != "$CONNSTR" ]; then
    echo "`date` - The Connection String has been changed! New Address: $CONNSTR" | $SSH root@$TMTSLV "cat >> /tmp/tmate-control.log"
    export TMATECONSTR="$CONNSTR"
    #exit 1
  else
    echo "`date` - The Connection String stay the same." | $SSH root@$TMTSLV "cat >> /tmp/tmate-control.log"
    #exit 0
  fi
}
